...
# grep 함수 : 숫자가 포함된 부분만 추출해달라!
grep("[[:digit:]]", text, value=T) # digit : 숫자

# gsub 함수 : "x"로 숫자 부분 변경!
gsub("[[:digit:]]", "x", text)

# grep 함수 : 한글만 추출하고 싶을 때!
grep("[ㄱ-핳]",text,value=T)

# grep 함수 : 영문소문자만 추출!
grep("[[:lower:]]", text, value=T) #lower : 영문소문자

# 정수로 쓴 표현만 인식
# 시작하는 숫자가 1-9 사이의 숫자, 띄어쓰고 뒤에 숫자는 0-9까지
...
# 블로거들이 추천하는 서울 명소들 워드 클라우드로 생성
# (서울명소추가 : 서울명소merge.txt)
# (제거단어 : 서울명소gsub.txt)
홈플러스 작전점
# setwd("c:\\r_temp")
# 필요 패키지 설치하는데 이미 설치 되어서 경고가 나오니 무시하세요.이화여자대학교
install.packages("KoNLP")
install.packages("wordcloud")
install.packages("stringr")
...
head(place, 10) # 상위 10개만 그냥 읽어들인 것
head(unlist(place),30) # unlist 형태로 30개 읽어들인 것

c <- unlist(place)
res <- str_replace_all(c,"[^[:alpha:]]", "") #공백 없이 바꿔치기!
...
# 두글자 이상인 단어만 걸러서(추출해서) 아래 파일로 저장
res2 <- Filter(function(x) {nchar(x) >= 2}, res)

# nrow 함수 : dataframe의 행의 수를 출력하는 함수
# nrow 함수는 데이터의 특정 열에 적용 할 수 없고, 벡터에도 적용할 수 없다.
nrow(res2)
